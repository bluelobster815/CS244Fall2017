function spo2 = spo2()
    ...
    num_full_windows = floor(length(IR_avg)/window_size);
    
    spo2s = zeros(1,num_full_windows+1);
    
    Rmeans = [];
    
    for j=1:num_full_windows+1
        
        end_index = j * window_size;
        start_index = end_index - window_size + 1;
        
        if j == num_full_windows+1
            IR_window = IR_avg(start_index:end);
            RED_window = RED_avg(start_index:end);
        else
