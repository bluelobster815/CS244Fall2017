...
activity='standing';
filename=strcat(activity,' data.csv');
fileout=strcat(activity,'_output.csv');

values = csvread(filename,10,0);

rr = respiration_rate_standing();
hr = heart_rate_standing();
spo2 = spo2_standing();
...
dlmwrite(fileout,data,'-append','precision','%d');
